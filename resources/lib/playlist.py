...
def _scan_directory_recursively(base_path):
    ...
    try:
        ...
        for dir_name in dirs:
            ...
            # Recursive call
            found_videos.extend(_scan_directory_recursively(sub_dir_path))
...
def get_playlist():
    ...
    if extra_folder_path and xbmcvfs.exists(extra_folder_path):
        ...
        try:
            playlist = _scan_directory_recursively(extra_folder_path)
            if playlist:
                shuffle(playlist)
                xbmc.log(f"Found {len(playlist)} videos. Playlist: {playlist}", level=xbmc.LOGDEBUG)
        except Exception as e:
